<x-layouts.admin title="Barang Ditemukan" subtitle="Kelola laporan barang ditemukan">
    {{-- Filter & Search --}}
    <div class="bg-white rounded-2xl shadow-sm border border-border p-6 mb-6">
        <form action="{{ route('admin.found-items.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                <select name="status" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition">
                    ...
                    <option value="Menunggu Pemilik" {{ request('status') == 'Menunggu Pemilik' ? 'selected' : '' }}>Menunggu Pemilik</option>
            ...
            <div class="md:col-span-4 flex gap-3">
                ...
                <a href="{{ route('admin.found-items.index') }}" class="px-6 py-2.5 bg-white border border-border text-gray-700 rounded-xl hover:bg-gray-50 transition">Reset</a>
                <a href="{{ route('admin.found-items.create') }}" class="ml-auto px-6 py-2.5 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 transition inline-flex items-center gap-2">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-border overflow-hidden hover:shadow-md transition">
                ...
                <div class="p-4">
                    <div class="flex items-start justify-between gap-2 mb-2">
                        ...
                        <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $item->status === 'Menunggu Pemilik' ? 'bg-emerald-100 text-emerald-700' : ($item->status === 'Diklaim' ? 'bg-primary-100 text-primary-700' : 'bg-blue-100 text-blue-700') }}">
                            {{ $item->status }}
                        </span>
                    </div>
                    <p class="text-xs text-text-secondary mb-1">{{ $item->category->name }} • {{ $item->location->name }}</p>
                    <p class="text-xs text-text-secondary mb-3">{{ $item->found_date->format('d M Y') }}</p>
                    
                    <div class="flex gap-2">
                        <a href="{{ route('admin.found-items.show', $item) }}" class="flex-1 px-3 py-2 text-xs font-medium text-primary-600 bg-primary-50 rounded-lg hover:bg-primary-100 transition text-center">Detail</a>
                        <a href="{{ route('admin.found-items.edit', $item) }}" class="flex-1 px-3 py-2 text-xs font-medium text-warning-600 bg-warning-50 rounded-lg hover:bg-warning-100 transition text-center">Edit</a>
                        <form action="{{ route('admin.found-items.destroy', $item) }}" method="POST" class="flex-1" onsubmit="return confirm('Yakin ingin menghapus?')">
                            @csrf
                            @method('DELETE')
