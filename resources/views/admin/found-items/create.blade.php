<x-layouts.admin title="Tambah Barang Ditemukan" subtitle="Buat laporan barang ditemukan baru">
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-border p-6">
            <form action="{{ route('admin.found-items.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="finder_name" value="{{ old('finder_name') }}" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition" required>
                            @error('finder_name') <p class="mt-1 text-sm text-danger-500">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Kelas</label>
                            <input type="text" name="class_name" value="{{ old('class_name') }}" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Nomor WhatsApp *</label>
                            <input type="text" name="phone_number" value="{{ old('phone_number') }}" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition" required>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="item_name" value="{{ old('item_name') }}" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition" required>
                        ...
                        <div>
                            ...
                            <input type="date" name="found_date" value="{{ old('found_date') }}" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition" required>
                        ...
                        <div>
                            ...
                            <select name="status" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition" required>
                                <option value="Menunggu Pemilik" {{ old('status') == 'Menunggu Pemilik' ? 'selected' : '' }}>Menunggu Pemilik</option>
                                <option value="Diklaim" {{ old('status') == 'Diklaim' ? 'selected' : '' }}>Diklaim</option>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="storage_location" value="{{ old('storage_location') }}" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition" placeholder="Contoh: Ruang BK, Laci Meja, dll">
                            @error('storage_location') <p class="mt-1 text-sm text-danger-500">{{ $message }}</p> @enderror
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi *</label>
                            <textarea name="description" rows="4" class="w-full px-4 py-2.5 rounded-xl border border-border focus:border-primary-500 outline-none transition" required>{{ old('description') }}</textarea>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-primary-500 rounded-xl hover:bg-primary-600">Simpan</button>
                </div>
            </form>
        </div>
    </div>
