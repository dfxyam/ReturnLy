<x-layouts.app title="Barang Ditemukan" active="found">
    <x-layout.container class="py-8 sm:py-12">
        ...
        {{-- Page Header --}}
        <div class="mb-8">
            <h1 class="text-3xl sm:text-4xl font-bold text-navy-800 mb-2">Barang Ditemukan</h1>
            <p class="text-text-secondary">Cari barang yang telah ditemukan di lingkungan sekolah dan ajukan klaim jika itu milik Anda.</p>
        </div>

        {{-- Search & Filter --}}
        <form method="GET" action="{{ route('found-items.index') }}" class="bg-white rounded-2xl shadow-sm border border-border p-6 mb-8">
            ...
            <div class="flex flex-col sm:flex-row gap-3 mt-6 pt-6 border-t border-border">
                ...
                <a href="{{ route('found-items.index') }}" class="inline-flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-semibold text-primary-500 bg-white border border-primary-500 hover:bg-primary-50 rounded-lg transition-all duration-200 h-[44px]">
                    Reset Filter
                </a>
            </div>
        </form>
        ...
        @else
            <x-shared.empty-state 
                ...
                description="Belum ada laporan barang ditemukan yang sesuai dengan pencarian Anda."
            />
        @endif
    </x-layout.container>
</x-layouts.app>